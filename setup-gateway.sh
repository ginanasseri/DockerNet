...
iptables -A FORWARD -i router-gateway -o gateway-nat -j ACCEPT

# Allow forwarding from nat-net to r-nat-net (response traffic)
iptables -A FORWARD -i gateway-nat -o router-gateway -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
